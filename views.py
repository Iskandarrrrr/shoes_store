...
class CategoryView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context['title'] = f'Bo`lim: {main_category.title}'
        return context

    def get_queryset(self):
        sort_field = self.request.GET.get('sort')  # Sartirofkaga javob beradi
        type_field = self.request.GET.get('type')  # Kategoriyaga tegishlilarini olib beradi

        if type_field:
            products = Product.objects.filter(category__slug=type_field)
            return products
        main_category = Category.objects.get(slug=self.kwargs['slug'])  # Glavniy kategoriyani
        subcategories = main_category.subcategories.all()  # kategoriyaga tegishli bolganlarini olib beradi !
...
def user_login(request):
    ...
    if form.is_valid():
        ...
        messages.error(request, 'Login yoki parolda xatolik !⚠️')
        return redirect('login_registration')


def user_logout(request):
    logout(request)
    messages.warning(request, 'Xayr !')
    return redirect('login_registration')
...
def register(request):
    ...
    if form.is_valid():
        user = form.save()
        messages.success(request, "Ro'yxatdan o'ttingiz ☺️!")
        return redirect('product_list')
    else:
        messages.error(request, "Xatolik! Boshqattan ko'rib chiqing!")
        return redirect('login_registration')
...
def to_cart(request, product_id, action):
    if request.user.is_authenticated:
        ...
        messages.error(request, 'Oldin royxatdan oting! ⚠️')
        return redirect('login_registration')
...
def successPayment(request):
    user_cart = CartForAuthenticatedUser(request)
    user_cart.clear()
    messages.success(request, 'Tolov muvaffaqiyatli boldi! 💸')
    return redirect('product_list')
